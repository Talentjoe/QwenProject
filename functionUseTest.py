...
def printLLMAOutputWithFunctionCall(outputString : str):
    if not DEBUG:
...
while (True):
    ...
    printLLMAOutputWithFunctionCall(output_text)

    while True :
        parse_result = try_parse_tool_calls(output_text)
        messages.append(parse_result[0])
        ...
        messages.append(try_parse_tool_calls(output_text)[0])
